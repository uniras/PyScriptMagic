...
def iframe_execute(line, cell, viewmode):
    ...
    width = args[1] if len(args) > 1 else "500"
...
{cell}
    ...
    if viewmode:
        ...
        def url_callback(url):
            # サーバーを起動
            start_server(temp_file_path, port)

            # ファイル名をURLに追加
            htmlurl = url + os.path.basename(temp_file_path)
...
def start_server_func(file_path, port):
    server_address = ('', port)
    httpd = HTTPServer(server_address, handler_factory(file_path))
